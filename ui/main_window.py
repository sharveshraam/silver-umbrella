...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        root=QWidget(); layout=QVBoxLayout(root); top=QHBoxLayout(); self.status=QLabel('IDLE | uptime 00:00 | reminders 0'); self.badge=QLabel('[VOICE]'); self.adb_badge=QLabel('[ADB: --- | WIFI: WAITING]')
        top.addWidget(self.badge); top.addWidget(self.adb_badge)
        self.buttons={}
    ...
    def set_adb_status(self, status: str) -> None:
        """Update the ADB/companion phone connection badge."""
        if hasattr(self, 'adb_badge'):
            self.adb_badge.setText(f"[{status}]")

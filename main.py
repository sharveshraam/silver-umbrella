...
from skills.adb_bridge import ADBBridge
from skills.companion_app_scaffold import CompanionBridge
...
def main() -> int:
    ...
    adb=ADBBridge(settings=settings, brain=brain, memory=memory); kg=KnowledgeGraph()
    ...
    def handle_text(text: str) -> None:
        ...
        tlow=text.lower(); skill=intent.get('skill','chat')
        if skill=='teacher': reply=brain.answer_with_teaching(text)
        elif skill=='self_learner': reply='Recent learnings: ' + str(learner.recent_learnings())
        elif skill=='focus_mode': start_focus(); return
        elif skill=='reminder':
            due=(dt.datetime.now()+dt.timedelta(hours=1)).isoformat(timespec='seconds'); rid=reminders.create(text, due); reply=f'Reminder saved (#{rid}).'
        elif 'phone' in tlow or 'adb' in tlow:
            reply=adb.handle(text)
        elif kg and ('timeline' in tlow or 'what was i working' in tlow):
            reply=kg.timeline_query(text)
        else: reply=brain.ask(text)
        if kg and settings.get('timeline_auto_log', True) and any(w in tlow for w in ('code','research','working on')):
            kg.add_event(skill, 'session', text[:240])
    ...
    companion = CompanionBridge(
        brain=brain,
        memory=memory,
        on_message=lambda text: QTimer.singleShot(0, lambda t=text: handle_text(t)),
        on_status=lambda msg: QTimer.singleShot(0, lambda m=msg: win.add_message("COPE", m))
    )
    app.aboutToQuit.connect(companion.stop)
    def check_connection_status():
        adb_status = adb.connection_status()
        wifi_status = companion.connection_status()
        win.set_adb_status(f"{adb_status} | {wifi_status}")
    conn_timer=QTimer(); conn_timer.setInterval(int(settings.get('adb_status_check_interval_seconds',15))*1000)
    conn_timer.timeout.connect(check_connection_status); conn_timer.start()
    win.show()
    if settings.get("companion_app_enabled", False):
        companion.start()
    check_connection_status()
    if settings.get("knowledge_graph_enabled", True):
        kg.add_event("session_start", "session", f"Started at {dt.datetime.now().isoformat(timespec='seconds')}")
    return app.exec()

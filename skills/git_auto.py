...
class _ChangeHandler(FileSystemEventHandler):
    ...
    def __init__(self, callback): self.callback = callback; self._pending = False
    def on_any_event(self, event):
        """Mark a pending change for non-directory, non-pyc files."""
        if not event.is_directory and not event.src_path.endswith(".pyc"): self._pending = True
class GitAuto:
    ...
    def start_watching(self, repo_path: str, debounce_seconds: int = 30) -> None:
        ...
        handler = _ChangeHandler(callback=None); self._observer = Observer(); self._observer.schedule(handler, repo_path, recursive=True); self._observer.start()
        def _debounce_loop():
            while True:
